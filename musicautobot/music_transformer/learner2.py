...
class MusicLearner(LanguageLearner):
    ...
    def predict(self, item:MusicItem, n_words:int=128,
                     ...
                     top_k=30, top_p=0.6, ref_measures=[], references=[]):
        ...
        for j_ in range(len(references)):
            ...
            for i_ in range(60):
                ...
                prev_idx = new_idx[-1][-1] if len(new_idx[-1]) else vocab.sep_idx
                ...
                logits = filter_invalid_indexes(logits, prev_idx, vocab, filter_value=filter_value)
                logits = filter_invalid_indices(vocab, logits, prev_idx, prev_pits, prev_durs, ref_measures[j_], references[j_], tot_len_so_far)
                ...
                tot_len = getLength(new_idx[-1], vocab)
                if tot_len == 16:
                    #print("innnnnnnnn" + str(prev_durs))
                    new_idx[-1].append(vocab.sep_idx)
                    break
                elif tot_len > 16:
                    #print("innnnn2 " + str(prev_durs))
                    new_idx[-1][-1] = 16 - getLength(new_idx[-1][:-1], vocab) + vocab.dur_range[0] - 1
...
def filter_invalid_indices(vocab, res, prev_idx, prev_pits, prev_durs, ref_measure, references, tot_len_so_far = 0):
    #print(prev_idx)
